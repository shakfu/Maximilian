...
maxiOsc myCounter,
    mySquare;     // these oscillators will help us count and play sound
int CurrentCount; // we're going to put the current count in this variable so
                  // that we can use it more easily.
...
void play(double* output)
{
    ...
    // Here you can see that CurrentCount is an int. It's taking the continuous
    // output of the phasor and convering it. You don't need to explicityly
    // 'cast' (i.e. change) the value from a float to an int. It happens
    // automagically in these cases.

    // Once every second, CurrentCount counts from 1 until it gets to 9, then
    ...
    // 100,200,300,400,500,600,700,800 in that order. These become the
    // frequency of the oscillator. In this case, the oscillator is an
    // antialiased sawtooth wave. Yum.
