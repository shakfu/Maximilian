...
// There are a number of filters in Maximilian, including low and high pass
// filters. There are also resonant filters and a state variable filter.
...
void play(double* output)
{
    ...
    // "{}". This is because there is only one outcome if the statement is
    // true.
    ...
    if (CurrentCount == 1)
        ...
        myEnvelope.trigger = 0; // release the envelope to make it fade out
                                // only if it's been triggered

    myOscOutput = mySwitchableOsc.sawn(100);

    // Below, the oscilator signals are being passed through a low pass filter.
    // The middle input is the filter cutoff. It is being controlled by the
    // envelope. Notice that the envelope is being amplified so that it scales
    // between 0 and 1000. The last input is the resonance.
