...
{ // some inits
    ...
    printf("Summary:\n%s",
           kick.getSummary()); // get info on samples if you like.
    // beats.getLength();
}
...
{ // this is where the magic happens. Very slow magic.
    ...
    if (lastCount != currentCount) { // if we have a new timer int this sample,
        ...
        kicktrigger = hit[playHead
                          % 16]; // get the value out of the array for the kick
    ...
    if (kicktrigger == 1) { // if the sequence has a 1 in it
        ...
        kick.trigger(); // reset the playback position of the sample to 0 (the
                        // beginning)
    }
    ...
    sampleOut = kick.playOnce()
        + snare.playOnce(); // just play the file. No looping.

    output[0] = sampleOut; // left channel
    output[1] = sampleOut; // right channel

    kicktrigger = 0; // set trigger to 0 at the end of each sample to guarantee
                     // retriggering.
    snaretrigger = 0;
}